...
| `/schedule` | 予定表(列=会場×行=時間帯のグリッド。`?day=N`で日付切替)。空き枠の範囲選択またはFAB→**モーダルでイベント作成** |
| `/items` | 持ち物リスト(足りない/調達予定/準備OK、持っていく/買ってくる)。FAB→**モーダルで追加** |
| `/expenses` | 費用一覧(合計・自分の負担・承認状況・精算表示)。FAB→**モーダルで費用追加** |
...
- [x] フェーズ4: 管理者コンソール — ロゴ登録 / 精算締め・精算リスト公開 / 管理者招待URL
- [x] フェーズ5: Vercel + Neon 本番デプロイ
