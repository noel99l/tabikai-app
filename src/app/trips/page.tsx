  ...
  return (
    <div className="mx-auto min-h-dvh max-w-md px-4">
      ...
      {memberships.map((m) => {
        ...
        return (
          <form key={m.tripId} action={selectTrip.bind(null, m.tripId)}>
            <button
              className={`mb-2.5 flex w-full items-center gap-3 rounded-xl border border-line bg-white p-3.5 text-left ${ended ? "opacity-60" : ""}`}
            >
              <span
