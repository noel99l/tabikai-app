...
// ありがとうポイント: 送る・送った履歴・(終了後)受け取ったポイントとメッセージ
    ...
    <>
      ...
      <Card className="mb-2.5 border-l-[6px] border-l-violet">
        <div className="flex items-center justify-between gap-2">
          ...
          {revealed ? (
            <Pill tone="violet">公開中</Pill>
          ) : (
            <Pill tone="pend">終了後に公開</Pill>
          )}
        </div>
        {revealed ? (
          <>
            ...
            {received.length === 0 ? (
              ...
              <div className="mt-2.5 flex flex-col gap-2">
                {received.map((r) => {
                  const from = memberOf(r.fromUserId);
                  return (
          ...
          <p className="mt-1 text-[12px] text-muted">
            みんなからのメッセージとポイントは、企画が終わる {fmtDateLabel(trip.endsAt)} 以降にここに表示されます。
          </p>
        )}
      </Card>
